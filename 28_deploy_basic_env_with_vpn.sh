...
main() {
    ...
    start_logging
    delete_everything
    create_zone "ubuntu_zone" # Creates global $ZONE_ID
...
init_vars() {
    ...
    IP_EXTERNAL_DNS="$IP_EXTERNAL_DNS"
    IP_HOME_DNS="10.8.0.1"
    IP_HOME_GATEWAY="10.8.0.1"
    ...
    UBUNTU_IP="10.8.0.1"
    UBUNTU_USER=root
    UBUNTU_PASSWORD=logon1vm
    UBUNTU_PRIMARY_STORAGE="nfs://$UBUNTU_IP/data/ubuntu_primary"
    UBUNTU_SECONDARY_STORAGE="nfs://$UBUNTU_IP/data/ubuntu_secondary"

    MAINFRAME_IP="10.8.0.2"
    MAINFRAME_USER=root
    MAINFRAME_PASSWORD=logon1vm
    MAINFRAME_PRIMARY_STORAGE="nfs://$MAINFRAME_IP/data/mainframe_primary"
    MAINFRAME_SECONDARY_STORAGE="nfs://$UBUNTU_IP/data/ubuntu_secondary"
...
create_zone() {
    ...
    do_cmd 'result=$(cmk create zone dns1='$IP_EXTERNAL_DNS' internaldns1='$IP_HOME_DNS' name='$zone_name' networktype=Basic)' "Zone created." "Zone creation failed."
...
add_primary_storage() {
    ...
    do_cmd 'result=$(cmk create storagepool zoneid='$zone_id' podid='$pod_id' clusterid='$cluster_id' name='$primary_storage_name' url='$UBUNTU_PRIMARY_STORAGE')'
...
add_secondary_storage () {
    ...
    do_cmd 'result=$(cmk add secondarystorage zoneid='$zone_id' url='$UBUNTU_SECONDARY_STORAGE')'
